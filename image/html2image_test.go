...
func TestHtml2image(t *testing.T) {
	...
	newObj.SetGlobalSettings([][2]string{
		{"out", ""},    // fmt can be omitted when out is not empty
		{"fmt", "png"}, // fmt must be specified when out is empty
		{"quality", "100"},
	})
	newObj.SetWidth("200").SetHeight("100")
	err, data := newObj.CreateImage(HTML)
	if err != nil {
		t.Fatal(err)
	}
	err = ioutil.WriteFile("sample.png", data, 0x777)
	if err != nil {
		t.Fatal(err)
	}

	newObj2 := New()
	defer newObj2.Destroy()

	// useinput file
	newObj2.SetGlobalSettings([][2]string{
		{"in", "sample.html"},
		{"out", ""},
		{"fmt", "png"},
		{"quality", "100"},
	})
	newObj2.SetHeight("100").SetWidth("200")
	err, data = newObj2.CreateImage("")
	if err != nil {
		t.Fatal(err)
	}
	err = ioutil.WriteFile("sample.html.png", data, 0x777)
	if err != nil {
		t.Fatal(err)
	}
}
